...
//int main_disparity(int argc, char** argv)
int FindDisparity(std::string strImageSrc, Mat& image)
{

	String left_im; // = strImageSrc;
	String right_im;// = strImageSrc
	String GT_path = "";

	String dst_path      = ".\\pics\\cm\\out_dst_movie.jpg";
	String dst_raw_path  = ".\\pics\\cm\\out_dst_raw_movie.jpg";
	String dst_conf_path = ".\\pics\\cm\\out_dst_conf_movie.jpg";
	String algo          = "sgbm";//"bm";
  ...
  //wsize = 15; //default window size for BM on full-sized views
	...
	Mat stereoImg = imread(strImageSrc, IMREAD_COLOR);
	if (stereoImg.empty()) { cout << "Cannot read image file: " << strImageSrc; getchar(); return -1; }
	...
	//if (left.empty()) { cout << "Cannot read image file: " << left_im; return -1; }
  // Setup a rectangle to define your region of interest
  cv::Rect leftROI(0, 0, 1280, 720);
	Mat left = stereoImg(leftROI);
	imshow("LEFT", left);
	
	//Mat right = imread(right_im, IMREAD_COLOR);
	//if (right.empty()) { cout << "Cannot read image file: " << right_im; return -1; }
  cv::Rect rightROI(1280, 0, 1280, 720);
	Mat right = stereoImg(rightROI);
  ...
  //waitKey(0);
	...
	Mat GT_disp;
	bool noGT;
  noGT = true; // no ground truth image file: " << GT_path << endl;
  ...
  cout << " dst_raw_path: " << dst_raw_path<< endl;
  ...
  cout << " dst_conf_path: " << dst_conf_path<< endl;
  ...
  if (!no_display)
	{
		...
		Mat imgSgbmDisparity8U = Mat(filtered_disp.rows, filtered_disp.cols, CV_8UC1);
		filtered_disp.convertTo(imgSgbmDisparity8U, CV_8UC1, 255 / (maxVal - minVal));
		namedWindow("imgSgbmDisparity8U", WINDOW_NORMAL);
		imshow("imgSgbmDisparity8U", imgSgbmDisparity8U);
		...
		while (1)
		{
			...
			if (key == 27 || key == 'q' || key == 'Q' || key == 32) // 'ESC'
				break;
		}
		//! [visualization]
	}
...
int FindDisparity1(std::string strImageSrc, Mat& image)
{

	Mat dispbm, dispsgbm, disparity, disparity1;

	Mat stereoImg = imread(strImageSrc, IMREAD_GRAYSCALE);
	if (stereoImg.empty()) { cout << "Cannot read image file: " << strImageSrc; return -1; }

	imshow("stereo", stereoImg);


	//! [load_views]
	Mat imgLeft = stereoImg(Rect(0, 0, 1280, 720));
	//imshow("LEFT", imgLeft);

	Mat imgRight = stereoImg(Rect(1280, 0, 1280, 720));
	...
	int ndisparities = 96; //16 * 5; //96;   /**< Range of disparity */
	int SADWindowSize = 7; //21; //7;
	...
	namedWindow("imgDisparity8U", WINDOW_NORMAL);

	imshow("imgDisparity8U", imgDisparity8U);


	//-- And create the image in which we will save our disparities
	Mat imgSgbmDisparity16S = Mat(imgLeft.rows, imgLeft.cols, CV_16S);
	Mat imgSgbmDisparity8U = Mat(imgLeft.rows, imgLeft.cols, CV_8UC1);

	Ptr<StereoSGBM> sgbm = StereoSGBM::create(-3,    //int minDisparity
		144,     //int numDisparities
		7,      //int SADWindowSize
		60,    //int P1 = 0
		2400,   //int P2 = 0
	...
	sgbm->compute(imgLeft, imgRight, imgSgbmDisparity16S);
	minMaxLoc(imgSgbmDisparity16S, &minVal, &maxVal);
	printf("SGBM Min disp: %f Max value: %f \n", minVal, maxVal);

	//-- 4. Display it as a CV_8UC1 image
	imgSgbmDisparity16S.convertTo(imgSgbmDisparity8U, CV_8UC1, 255 / (maxVal - minVal));
	...
	//waitKey(0);
	//imshow("dispbm", dispbm);

	//getchar();
}

int Disparity(Mat &stereoImg)
{
...
//
  ...
  if (!no_display)
    ...
    Mat edge;
    cv::Canny(imgSgbmDisparity8U, edge, 0, 50, 5);
    imshow("Edge", edge);
...
// Reference: https://searchcode.com/codesearch/view/68960453/
